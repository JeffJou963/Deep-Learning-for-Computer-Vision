...
# nn.Linear -> lora.Linear(in, out, r=16)
...
#         self.down = nn.Linear(cfg.n_embd, 100)
#         self.gelu = nn.GELU(approximate='tanh')
#         self.up = nn.Linear(100, cfg.n_embd)
#         # self.LN = nn.LayerNorm(cfg.n_embd)

#     def forward(self, x):
#         x = x + self.up(self.gelu(self.down(x)))
#         # x = x + self.LN(self.up(self.gelu(self.down(x))))
#         return x
...
#         out = self.c_proj((att @ v).transpose(1, 2).contiguous().view(B, T, C))
...
class Attention(nn.Module):
    ...
    def __init__(self, cfg):
        super().__init__()
        # self.c_attn = nn.Linear(cfg.n_embd, 3 * cfg.n_embd)
        # self.lora_attn = lora.MergedLinear(cfg.n_embd, 3*cfg.n_embd, r=16, enable_lora=[True,False,True])
        self.lora_attn = lora.Linear(cfg.n_embd, 3*cfg.n_embd, r=16)
...
class Cross_Attention(nn.Module):
    ...
    def forward(self, x, encoder_output):
        ...
        viz = (q @ k.transpose(-2, -1))
        att = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))
        att = F.softmax(att, dim=-1)  #(1,12, 257,257)
        # viz = att
        # att@v.T = (1,257,12,64)
        return self.c_proj((att @ v).transpose(1, 2).contiguous().view(B, T, C)), viz #(1,257,768)
...
class Block(nn.Module):
    ...
    def forward(self, x, encoder_output):
        ...
        cross, viz = self.cross_attn(self.ln_2(x), encoder_output)
        x = x + cross
...
if __name__ == '__main__':
    ...
    model = Transformer_lora(encoder_path, decoder_cfg_path)
