...
class PrefixTuning(nn.Module):
    def __init__(self, cfg):
        ...
        self.prefix_embedding = nn.Embedding(100, cfg.n_embd)

    def forward(self, x, prefix_idx):
        return x + self.prefix_embedding(prefix_idx)

class Decoder_tuning(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.prefix_tuning = PrefixTuning(cfg)
    ...
    def forward(self, encoder_output, caption_in):
        ...
        tuning = torch.rand(caption_in.size(0),10, device = caption_in.device)
        # x = torch.narrow(caption_in, 1, 0, min(caption_in.size(1), self.block_size))    #(1,max_cap_len)
        x = torch.narrow(caption_in, 1, 0, (min(caption_in.size(1), self.block_size)-10))    #(1,max_cap_len)
        x = torch.cat([tuning, x], dim=1).to(torch.int64)
...
class Transformer_tuning(nn.Module):
    def __init__(self, encoder_path, decoder_cfg_path):
        ...
        self.decoder = Decoder_tuning(decoder_cfg)
