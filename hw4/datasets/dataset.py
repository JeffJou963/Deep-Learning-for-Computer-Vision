...
class KlevrDataset(Dataset):
    def __init__(self, json_dir, split='train', get_rgb=True) -> None:
        # super().__init__()
        '''
    ...
    def read_meta(self):
        with open(os.path.join(self.json_dir, 'metadata.json'), "r") as f:
        ...
        if self.split == 'train':
            ...
            for image_id in self.split_ids:
                if self.get_rgb:
                    image_path = os.path.join(self.json_dir, f'{image_id:05d}.png')
    ...
    def __getitem__(self, idx):
        ...
        # split of val/test
            ...
            if self.get_rgb:
                img = Image.open(os.path.join(self.json_dir, f'{image_id:05d}.png'))
                img = img.resize(self.img_wh, Image.Resampling.LANCZOS)
